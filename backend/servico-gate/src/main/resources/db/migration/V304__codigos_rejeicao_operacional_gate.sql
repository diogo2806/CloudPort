...
BEGIN
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION USING
            ERRCODE = 'P1003',
            MESSAGE = 'GATE_DRIVER_PERMISSION_REQUIRED';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION USING
            ERRCODE = 'P1003',
            MESSAGE = 'GATE_CARRIER_PERMISSION_REQUIRED';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION USING
            ERRCODE = 'P1003',
            MESSAGE = 'GATE_VEHICLE_PERMISSION_REQUIRED';
    END IF;
    ...
    IF NEW.agendamento_id IS NOT NULL THEN
        ...
        IF v_janela_id IS NULL THEN
            RAISE EXCEPTION USING
                ERRCODE = 'P1001',
                MESSAGE = 'GATE_WINDOW_CAPACITY_EXHAUSTED';
        END IF;
...
BEGIN
    ...
    IF NEW.bill_of_lading_id IS NOT NULL THEN
        ...
        IF NOT v_bl_disponivel THEN
            RAISE EXCEPTION USING
                ERRCODE = 'P1004',
                MESSAGE = 'GATE_REFERENCE_UNAVAILABLE';
        END IF;
    END IF;
    ...
    IF NEW.status = 'CONCLUIDA'
       ...
       AND NEW.bill_of_lading_id IS NOT NULL THEN
        ...
        IF v_atualizados = 0 THEN
            RAISE EXCEPTION USING
                ERRCODE = 'P1004',
                MESSAGE = 'GATE_REFERENCE_BALANCE_EXHAUSTED';
        END IF;
    END IF;
