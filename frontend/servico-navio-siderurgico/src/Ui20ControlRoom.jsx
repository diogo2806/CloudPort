...
import { EMPTY_INTEGRATION, EMPTY_SUMMARY, PHASES, TRANSITION_ACTIONS, clean, dateTime, normalized, number, orderCode, orderDestination, orderOrigin, planDraft, serializePlan, statusClass, validatePlan } from './ui20-model.js';
...
export default function Ui20ControlRoom({ session, onLogout }) {
  ...
  const load = useCallback(async (id, silent = false, forcePlan = false) => {
    ...
    try {
      const values = await Promise.all([
        ...
        api.obterQuayMonitor(id), craneApi.obterPlanoGuindaste(id), api.listarJobListsEquipamentoPatio(id), api.obterMatrizEstadosWorkInstructionPatio(), api.listarTelemetriaEquipamentos()
      ]);
      ...
      setEdits((current) => (newQueues || []).reduce((acc, queue) => ({ ...acc, [queue.id]: current[queue.id] || { pow: queue.pow || '', poolOperacional: queue.poolOperacional || '', porao: queue.porao || '', planoGuindasteId: queue.planoGuindasteId || '', recursoCaisId: queue.recursoCaisId || '', equipamentoPatioId: queue.equipamentoPatioId || '' } }), {}));
  ...
  async function savePlan() {
    ...
    const saved = await action('plan', () => craneApi.salvarPlanoGuindaste(visitId, serializePlan(draft)), 'Plano de guindastes salvo.', false);
  ...
  async function saveResources(queue) {
    ...
    await action(`resources-${queue.id}`, () => api.atualizarRecursosWorkQueuePatio(queue.id, {
      pow: clean(edit.pow) || null, poolOperacional: clean(edit.poolOperacional) || null, porao: edit.porao ? Number(edit.porao) : null, planoGuindasteId: edit.planoGuindasteId ? Number(edit.planoGuindasteId) : null,
  ...
  async function queueAction(queue, type) {
    const operations = {
      activate: () => api.ativarWorkQueuePatio(queue.id, 'Ativação pelo Control Room'),
      deactivate: () => api.desativarWorkQueuePatio(queue.id, 'Desativação pelo Control Room'),
      dispatch: () => api.despacharWorkQueuePatio(queue.id, { limiteOrdens: null, observacao: 'Dispatch pelo Control Room' })
    };
    await action(`queue-${queue.id}`, operations[type], type === 'dispatch' ? 'Dispatch executado.' : 'Work queue atualizada.');
  }
  async function instructionAction(order, type) {
    const operation = type === 'reset'
      ? () => api.resetarWorkInstructionPatio(order.id, 'Reset pelo Control Room')
      : () => api.cancelarWorkInstructionPatio(order.id, 'Cancelamento pelo Control Room');
    await action(`instruction-${order.id}`, operation, type === 'reset' ? 'Work instruction resetada.' : 'Work instruction cancelada.');
  }
  ...
  return <div className="app"><header className="topbar"><div><span className="eyebrow">CloudPort</span><h1>Control Room Navio + Pátio</h1></div><div className="stream-health"><Stream value={stream} /><Stream value={telemetryStream} /></div><div className="top-actions"><span>{session.nome}</span><button className="secondary" onClick={() => load(visitId, false)}>Atualizar</button><button className="danger" onClick={onLogout}>Sair</button></div></header><main className="content">
    ...
    <Ui10VisitPlanSection visit={selectedVisit} onVisitPersisted={(updated) => setVisits((current) => current.map((visit) => Number(visit.id) === Number(updated.id) ? updated : visit))} />
    ...
    <section className="panel"><div className="section-head"><h2>Ações e filtros</h2></div><div className="actions">{PHASES[selectedVisit?.fase] && <button onClick={() => action('phase', () => api.alterarFaseVisita(visitId, PHASES[selectedVisit.fase], `Avanço operacional para ${PHASES[selectedVisit.fase]}`), `Visita avançada para ${PHASES[selectedVisit.fase]}.`)}>Avançar para {PHASES[selectedVisit.fase]}</button>}<button onClick={() => action('reserve', () => api.gerarReservasPatio(visitId), 'Reservas geradas.')}>Gerar reservas</button><button onClick={() => action('orders', () => api.gerarOrdensPatio(visitId), 'Ordens geradas.')}>Gerar ordens</button><button className="secondary" onClick={() => action('sync', () => api.sincronizarStatusPatio(visitId), 'Yard sincronizado.')}>Sincronizar Yard</button><button className="secondary" onClick={() => action('optimize', () => api.otimizarOperacaoGlobal(visitId), 'Otimização global executada.')}>Otimizar operação</button><button className="secondary" onClick={() => api.baixarRelatorioCsv(visitId).catch((reason) => setError(formatError(reason)))}>CSV</button><button className="secondary" onClick={() => api.baixarRelatorioPdf(visitId).catch((reason) => setError(formatError(reason)))}>PDF</button></div><div className="filters"><label>Status<select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="">Todos</option>{['ATIVA','PENDENTE','EM_EXECUCAO','BLOQUEADA','SUSPENSA','CONCLUIDA','CANCELADA'].map((value) => <option key={value}>{value}</option>)}</select></label><label>Bloco/zona<input value={zoneFilter} onChange={(event) => setZoneFilter(event.target.value)} /></label><label>Severidade<select value={severityFilter} onChange={(event) => setSeverityFilter(event.target.value)}><option value="">Todas</option>{['BAIXA','MEDIA','ALTA','CRITICA'].map((value) => <option key={value}>{value}</option>)}</select></label></div></section>
