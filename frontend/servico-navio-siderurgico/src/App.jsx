...
const orderCode = (order) => order?.codigoLote || order?.codigoConteiner || `Ordem ${order?.id ?? '—'}`;
...
function OrdersTable({ orders, priorities, busyKey, readOnly, onPriority, onSuspend, onResume, onDetails }) {
  ...
  return <div className="table-wrap"><table><thead><tr><th>Lote</th><th>Status</th><th>Origem</th><th>Destino</th><th>Seq.</th><th>Prioridade</th><th>Ações</th></tr></thead><tbody>{orders.map((order) => <tr key={order.id ?? `${orderCode(order)}-${order.sequenciaNavio}`}>
    <td><strong>{orderCode(order)}</strong><small>{order.tipoMovimento}</small></td><td><span className={statusClass(order.statusOrdem)}>{order.statusOrdem}</span></td><td>{order.origem || order.tipoOrigem || '—'}</td><td>{order.destino || order.posicaoPlanejada || '—'}</td><td>{order.sequenciaNavio ?? '—'}</td>
...
function WorkQueue({ queue, expanded, edit, priorities, fetchPriorities, busyKey, onToggle, onEdit, onAction, onInstruction, onDetails }) {
  ...
  return <article className={`queue ${derived ? 'queue-derived' : ''}`}><button className="queue-head" onClick={onToggle} aria-expanded={expanded}>
    <span><strong>{queue.identificador}</strong><small>{queue.berco || 'sem berço'} · porão {queue.porao ?? '—'} · {queue.blocoZona || 'sem zona'}</small></span><span>{derived && <ExceptionBadge type="SEM_FILA" />}<span className={statusClass(queue.status)}>{queue.status}</span><strong>{jobs.length || queue.totalOrdens || 0} jobs</strong>{expanded ? '▴' : '▾'}</span>
  </button>{expanded && <div className="queue-body">
    {!derived && <><div className="queue-editor">
      <label>POW<input value={edit.pow} onChange={(event) => onEdit({ ...edit, pow: event.target.value })} /></label><label>Pool<input value={edit.poolOperacional} onChange={(event) => onEdit({ ...edit, poolOperacional: event.target.value })} /></label><button className="secondary" onClick={() => onAction('pow')}>Salvar POW/pool</button>
      <label>Porão<input type="number" min="0" value={edit.porao ?? ''} onChange={(event) => onEdit({ ...edit, porao: event.target.value ? Number(event.target.value) : null })} /></label><label>Plano de guindaste<input type="number" min="1" value={edit.planoGuindasteId ?? ''} onChange={(event) => onEdit({ ...edit, planoGuindasteId: event.target.value ? Number(event.target.value) : null })} /></label><label>Recurso de cais<input type="number" min="1" value={edit.recursoCaisId ?? ''} onChange={(event) => onEdit({ ...edit, recursoCaisId: event.target.value ? Number(event.target.value) : null })} /></label><label>CHE real<input type="number" min="1" value={edit.equipamentoPatioId ?? ''} onChange={(event) => onEdit({ ...edit, equipamentoPatioId: event.target.value ? Number(event.target.value) : null })} /></label><button className="secondary" onClick={() => onAction('resources')}>Salvar recursos</button>
    ...
    </div><p className="empty">Guindaste: {queue.planoGuindasteId ?? 'não associado'} · recurso de cais: {queue.recursoCaisId ?? 'não associado'} · CHE: {queue.equipamento || queue.equipamentoPatioId || 'não associado'}</p><div className="actions"><button className={queue.status === 'ATIVA' ? 'warning' : ''} onClick={() => onAction(queue.status === 'ATIVA' ? 'deactivate' : 'activate')}>{queue.status === 'ATIVA' ? 'Desativar' : 'Ativar'}</button><button disabled={busyKey === `dispatch-${queue.id}`} onClick={() => onAction('dispatch')}>Despachar</button></div></>}
    {jobs.length ? <div className="table-wrap nested"><table><thead><tr><th>Lote</th><th>Status</th><th>Destino</th><th>Prioridade operacional</th><th>Fetch</th><th>Ações</th></tr></thead><tbody>{jobs.map((job) => <tr key={job.id}><td>{orderCode(job)}</td><td><span className={statusClass(job.statusOrdem)}>{job.statusOrdem}</span></td><td>{job.destino || job.posicaoPlanejada || '—'}</td><td>{derived ? priorities[job.id] ?? job.prioridadeOperacional ?? 0 : <input type="number" min="0" value={priorities[job.id] ?? job.prioridadeOperacional ?? 0} onChange={(event) => onInstruction('edit-priority', job, Number(event.target.value))} />}</td><td>{derived ? (job.prioridadeBusca ? 'Prioritário' : 'Normal') : <label className="check"><input type="checkbox" checked={fetchPriorities[job.id] ?? !!job.prioridadeBusca} onChange={(event) => onInstruction('edit-fetch', job, event.target.checked)} />Priorizar</label>}</td><td><div className="actions compact">
      <button className="small secondary" onClick={() => onDetails(job)}>Detalhes</button>{!derived && <><button className="small secondary" onClick={() => onInstruction('priority', job)}>Salvar prioridades</button>{!terminal(job.statusOrdem) && job.statusOrdem !== 'SUSPENSA' && <button className="small warning" onClick={() => onInstruction('suspend', job)}>Suspender</button>}{['SUSPENSA', 'BLOQUEADA'].includes(job.statusOrdem) && <button className="small" onClick={() => onInstruction('resume', job)}>Retomar</button>}{!terminal(job.statusOrdem) && job.statusOrdem !== 'BLOQUEADA' && <button className="small warning" onClick={() => onInstruction('block', job)}>Bloquear</button>}{job.statusOrdem === 'EM_EXECUCAO' && <button className="small" onClick={() => onInstruction('complete', job)}>Concluir</button>}{!terminal(job.statusOrdem) && <button className="small secondary" onClick={() => onInstruction('reset', job)}>Resetar</button>}{!terminal(job.statusOrdem) && <button className="small danger" onClick={() => onInstruction('cancel', job)}>Cancelar</button>}</>}
    </div></td></tr>)}</tbody></table></div> : <p className="empty">A fila não possui jobs.</p>}
  </div>}</article>;
}
...
function ExceptionPanel({ alerts, uncovered }) {
  ...
  return <section className="panel"><div className="section-head"><div><span className="eyebrow">Exceções</span><h2>Mapa de exceções operacionais</h2></div><span>{alerts.length + uncovered.length}</span></div><div className="exception-grid">{Object.entries(EXCEPTIONS).map(([type, meta]) => <article key={type} className={`exception-card exception-${type.toLowerCase().replaceAll('_', '-')}`}><div><ExceptionBadge type={type} /><strong>{meta[1]}</strong><b>{grouped[type].length}</b></div><p>{meta[2]}</p>{grouped[type].slice(0, 2).map((entry, index) => <small key={`${type}-${entry.id ?? index}`}>{entry.mensagem || orderCode(entry)}</small>)}</article>)}</div></section>;
}

function EquipmentPanel({ panels, queues, onDetails }) {
  const fallback = useMemo(() => {
    const result = new Map();
    queues.forEach((queue) => {
      const id = queue.equipamento || 'SEM_EQUIPAMENTO';
      const item = result.get(id) ?? { equipamentoPatioId: id, equipamentoIdentificador: id, equipamentoTipo: 'NÃO INFORMADO', equipamentoStatus: id === 'SEM_EQUIPAMENTO' ? 'SEM_EQUIPAMENTO' : 'ASSOCIADO', workQueues: [], totalFilas: 0, totalInstrucoes: 0 };
      item.workQueues.push(queue); item.totalFilas += 1; item.totalInstrucoes += (queue.jobList ?? []).length; result.set(id, item);
    });
    return [...result.values()];
  }, [queues]);
  const groups = panels.length ? panels : fallback;
  return <section className="panel"><div className="section-head"><div><span className="eyebrow">CHE</span><h2>Equipamentos e job lists</h2></div><span>{groups.length}</span></div><div className="equipment-grid">{groups.map((group) => {
    const jobs = (group.workQueues ?? []).flatMap((queue) => queue.jobList ?? []);
    const missing = !group.equipamentoPatioId || group.equipamentoIdentificador === 'SEM_EQUIPAMENTO';
    return <article key={group.equipamentoPatioId ?? group.equipamentoIdentificador} className={`equipment-card ${missing ? 'equipment-missing' : ''}`}><div className="equipment-head"><div><strong>{group.equipamentoIdentificador || 'SEM_EQUIPAMENTO'}</strong><small>{group.equipamentoTipo} · {group.equipamentoStatus}</small></div>{missing && <ExceptionBadge type="SEM_EQUIPAMENTO" />}</div><div className="equipment-metrics"><span><b>{group.totalInstrucoes ?? jobs.length}</b> jobs</span><span><b>{jobs.filter((job) => job.statusOrdem === 'EM_EXECUCAO').length}</b> executando</span><span><b>{group.totalFilas ?? group.workQueues?.length ?? 0}</b> filas</span></div><div className="equipment-jobs">{jobs.slice(0, 8).map((job) => <button className="job-chip" key={job.id} onClick={() => onDetails(job)}>#{job.sequenciaNavio ?? '—'} {orderCode(job)}<small>{job.statusOrdem}</small></button>)}{!jobs.length && <p className="empty">Sem jobs.</p>}</div></article>;
  })}</div></section>;
}
...
function InstructionDrawer({ context, onClose }) {
  ...
  const audit = drillDown?.auditoria ?? [];
  return <div className="drawer-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><aside className="instruction-drawer" role="dialog" aria-modal="true"><div className="drawer-head"><div><span className="eyebrow">Work instruction</span><h2>{orderCode(order)}</h2></div><button className="secondary" onClick={onClose}>Fechar</button></div><div className="detail-grid">
    <article><span>Status</span><strong className={statusClass(order.statusOrdem)}>{order.statusOrdem}</strong></article><article><span>Work queue</span><strong>{drillDown?.workQueue?.identificador || queue?.identificador || 'Sem fila'}</strong></article><article><span>POW / CHE</span><strong>{drillDown?.workQueue?.pow || queue?.pow || 'sem POW'} · {drillDown?.equipamentoIdentificador || queue?.equipamento || 'sem CHE'}</strong></article><article><span>Sequência</span><strong>{order.sequenciaNavio ?? '—'}</strong></article><article><span>Origem</span><strong>{order.origem || order.tipoOrigem || '—'}</strong></article><article><span>Destino planejado</span><strong>{order.destino || order.posicaoPlanejada || '—'}</strong></article><article><span>Posição real</span><strong>{order.posicaoReal || item?.posicaoPatioReal || '—'}</strong></article><article><span>Reserva</span><strong>{reservation ? `${reservation.posicaoPatioId} · ${reservation.status}` : 'Sem reserva'}</strong></article><article><span>Prioridade operacional</span><strong>{order.prioridadeOperacional ?? 0}</strong></article><article><span>Prioridade de fetch</span><strong>{order.prioridadeBusca ? 'PRIORITÁRIA' : 'NORMAL'}</strong></article>
  </div><section className="drawer-section"><h3>Próximos estados permitidos</h3><p>{(drillDown?.proximosEstadosPermitidos ?? []).join(', ') || 'Nenhum estado disponível.'}</p></section><section className="drawer-section"><h3>Item de navio</h3>{item ? <pre>{JSON.stringify(item, null, 2)}</pre> : <p className="empty">Item não localizado.</p>}</section><section className="drawer-section"><h3>Divergências</h3><div className="list">{divergences.map((entry, index) => <article key={`${entry.tipo}-${index}`}><div><strong>{entry.tipo}</strong><span className={statusClass(entry.severidade || 'ALTA')}>{entry.severidade || 'ALTA'}</span></div><p>{entry.mensagem}</p></article>)}{!divergences.length && <p className="empty">Nenhuma divergência.</p>}</div></section><section className="drawer-section"><h3>Auditoria da work instruction</h3><div className="timeline">{audit.map((entry) => <article key={entry.id}><time>{dateTime(entry.criadoEm)}</time><div><strong>{entry.acao}</strong><p>{entry.motivo || 'Sem motivo informado.'}</p><small>{entry.usuario} · {entry.detalhes || 'sem detalhes'}</small></div></article>)}{!audit.length && <p className="empty">Nenhum registro de auditoria.</p>}</div></section><section className="drawer-section"><h3>Eventos relacionados</h3><div className="timeline">{relatedEvents.map((event) => <article key={event.id}><time>{dateTime(event.criadoEm)}</time><div><strong>{event.tipoEvento}</strong><p>{event.descricao}</p><small>{event.usuario}</small></div></article>)}{!relatedEvents.length && <p className="empty">Nenhum evento relacionado.</p>}</div></section><section className="drawer-section"><h3>Alertas relacionados</h3><div className="list">{relatedAlerts.map((alert, index) => <article key={`${alert.tipo}-${index}`}><div><strong>{alert.tipo}</strong><span className={statusClass(alert.severidade)}>{alert.severidade}</span></div><p>{alert.mensagem}</p></article>)}{!relatedAlerts.length && <p className="empty">Nenhum alerta relacionado.</p>}</div></section></aside></div>;
}
...
function ControlRoom({ session, onLogout }) {
  ...
  const [items, setItems] = useState([]), [events, setEvents] = useState([]), [reservations, setReservations] = useState([]), [orders, setOrders] = useState([]), [queues, setQueues] = useState([]), [workQueues, setWorkQueues] = useState([]), [uncovered, setUncovered] = useState([]), [alerts, setAlerts] = useState([]), [equipmentPanels, setEquipmentPanels] = useState([]);
  const [statusFilter, setStatusFilter] = useState(''), [zoneFilter, setZoneFilter] = useState(''), [severityFilter, setSeverityFilter] = useState('');
  const [lastUpdate, setLastUpdate] = useState(null), [streamStatus, setStreamStatus] = useState('CONECTANDO'), [expanded, setExpanded] = useState({}), [edits, setEdits] = useState({}), [priorities, setPriorities] = useState({}), [fetchPriorities, setFetchPriorities] = useState({}), [selected, setSelected] = useState(null), [drillDown, setDrillDown] = useState(null);
  ...
  const loadSnapshot = useCallback(async (id, silent = false) => {
    ...
    const promise = Promise.all([api.listarItensVisita(id), api.obterResumo(id), api.listarEventos(id), api.obterResumoIntegracaoPatio(id), api.listarReservasPatio(id), api.listarOrdensPatio(id), api.listarFilasPatio(id), api.listarWorkQueuesPatio(id), api.listarOrdensSemCoberturaPatio(id), api.listarAlertasIntegracaoPatio(id), api.listarJobListsEquipamentoPatio(id)]).then(([a, b, c, d, e, f, g, h, i, j, k]) => {
      ...
      setItems(a); setSummary(b); setEvents(c); setIntegration(d); setReservations(e); setOrders(f); setQueues(g); setWorkQueues(h); setUncovered(i); setAlerts(j); setEquipmentPanels(k); setLastUpdate(new Date());
      const allOrders = [...f, ...h.flatMap((queue) => queue.jobList ?? [])];
      setPriorities((current) => allOrders.reduce((acc, order) => order.id ? { ...acc, [order.id]: current[order.id] ?? order.prioridadeOperacional ?? 0 } : acc, {}));
      setFetchPriorities((current) => allOrders.reduce((acc, order) => order.id ? { ...acc, [order.id]: current[order.id] ?? !!order.prioridadeBusca } : acc, {}));
      setEdits((current) => h.reduce((acc, queue) => ({ ...acc, [queue.id]: current[queue.id] ?? { pow: queue.pow || '', poolOperacional: queue.poolOperacional || '', porao: queue.porao ?? null, planoGuindasteId: queue.planoGuindasteId ?? null, recursoCaisId: queue.recursoCaisId ?? null, equipamentoPatioId: queue.equipamentoPatioId ?? null, limite: null } }), {}));
  ...
  useEffect(() => {
    ...
    const connect = async () => { if (stopped) return; setStreamStatus('CONECTANDO'); try { await api.assinarControlRoom(visitId, { signal: controller.signal, onOpen: () => setStreamStatus('ONLINE'), onEvent: ({ data }) => data?.tipo === 'SNAPSHOT_INVALIDADO' && loadSnapshot(visitId, true).catch((reason) => setError(formatError(reason))) }); } catch (reason) { if (stopped || controller.signal.aborted) return; setStreamStatus('RECONECTANDO'); setError(formatError(reason)); timer = setTimeout(connect, 3000); } };
  ...
  async function queueAction(queue, type) {
    ...
    const operations = {
      ...
      dispatch: () => api.despacharWorkQueuePatio(queue.id, { limiteOrdens: edit.limite || null, observacao: 'Dispatch pelo Control Room' })
    };
  ...
  async function openDetails(order) {
    setSelected(order); setDrillDown(null);
    try { setDrillDown(await api.obterDrillDownWorkInstructionPatio(order.id)); } catch (reason) { setError(formatError(reason)); }
  }
  ...
  const filteredQueues = workQueues.filter((queue) => (!statusFilter || queue.status === statusFilter) && (!zoneFilter || normalize(`${queue.identificador} ${queue.berco} ${queue.blocoZona} ${queue.pow} ${queue.equipamento}`).includes(normalize(zoneFilter))));
  ...
  const context = useMemo(() => {
    ...
    const item = items.find((entry) => entry.id === order.itemOperacaoNavioId), reservation = reservations.find((entry) => entry.itemOperacaoNavioId === order.itemOperacaoNavioId), queue = workQueues.find((entry) => (entry.jobList ?? []).some((job) => job.id === order.id));
    const relatedAlerts = alerts.filter((entry) => entry.ordemTrabalhoPatioId === order.id || entry.itemOperacaoNavioId === order.itemOperacaoNavioId), needles = [order.id, order.itemOperacaoNavioId, orderCode(order)].filter(Boolean).map(String), relatedEvents = events.filter((entry) => needles.some((needle) => `${entry.tipoEvento} ${entry.descricao}`.includes(needle))), divergences = [...relatedAlerts];
    ...
    if (!queue) divergences.push({ tipo: 'SEM_FILA', severidade: 'ALTA', mensagem: 'Work instruction sem fila persistente.' }); else { if (!queue.pow) divergences.push({ tipo: 'SEM_POW', severidade: 'ALTA', mensagem: 'Fila sem POW.' }); if (!queue.equipamentoPatioId && !queue.equipamento) divergences.push({ tipo: 'SEM_EQUIPAMENTO', severidade: 'ALTA', mensagem: 'Fila sem CHE.' }); }
    return { order, item, reservation, queue, relatedAlerts, relatedEvents, divergences, drillDown };
  }, [selected, orders, workQueues, uncovered, items, reservations, alerts, events, drillDown]);
  ...
  return <div className="app"><header className="topbar"><div><span className="eyebrow">CloudPort</span><h1>Control Room Navio + Pátio</h1></div><div className="top-actions"><StreamStatus value={streamStatus} /><span>{session.nome}</span><button className="secondary" onClick={() => action('refresh', () => loadSnapshot(visitId), 'Atualizado.')}>Atualizar</button><button className="danger" onClick={onLogout}>Sair</button></div></header><main className="content">
    ...
    <section className="panel"><div className="section-head"><div><span className="eyebrow">Execução</span><h2>Movimentos iminentes</h2></div><span>{imminent.length}</span></div><div className="imminent">{imminent.map((order) => <article key={order.id} onClick={() => openDetails(order)}><span>#{order.sequenciaNavio ?? '—'}</span><strong>{orderCode(order)}</strong><small>{order.origem || order.tipoOrigem || '—'} → {order.destino || order.posicaoPlanejada || '—'}</small><span className={statusClass(order.statusOrdem)}>{order.statusOrdem}</span></article>)}</div></section>
    <QuayMonitor visit={selectedVisit} queues={filteredQueues} alerts={filteredAlerts} /><EquipmentPanel panels={equipmentPanels} queues={filteredQueues} onDetails={openDetails} /><ExceptionPanel alerts={filteredAlerts} uncovered={uncovered} />
    <section className="panel"><div className="section-head"><div><span className="eyebrow">Equipment Control</span><h2>Work queues e job lists</h2></div><span>{filteredQueues.length}</span></div>{filteredQueues.map((queue) => <WorkQueue key={queue.id ?? queue.identificador} queue={queue} expanded={!!expanded[queue.id ?? queue.identificador]} edit={edits[queue.id] ?? { pow: '', poolOperacional: '', porao: null, planoGuindasteId: null, recursoCaisId: null, equipamentoPatioId: null, limite: null }} priorities={priorities} fetchPriorities={fetchPriorities} busyKey={busyKey} onToggle={() => setExpanded((current) => ({ ...current, [queue.id ?? queue.identificador]: !current[queue.id ?? queue.identificador] }))} onEdit={(edit) => setEdits((current) => ({ ...current, [queue.id]: edit }))} onAction={(type) => queueAction(queue, type)} onInstruction={instructionAction} onDetails={openDetails} />)}</section>
    <section className="panel"><div className="section-head"><div><span className="eyebrow">Yard</span><h2>Ordens de pátio</h2></div><span>{filteredOrders.length}/{orders.length}</span></div><OrdersTable orders={filteredOrders} priorities={priorities} busyKey={busyKey} onPriority={changePriority} onDetails={openDetails} onSuspend={(order) => action(`suspend-${order.id}`, () => api.suspenderOrdemPatio(visitId, order.id), 'Ordem suspensa.')} onResume={(order) => action(`resume-${order.id}`, () => api.retomarOrdemPatio(visitId, order.id), 'Ordem retomada.')} /></section>
  ...
  </main><InstructionDrawer context={context} onClose={() => { setSelected(null); setDrillDown(null); }} /></div>;
}
