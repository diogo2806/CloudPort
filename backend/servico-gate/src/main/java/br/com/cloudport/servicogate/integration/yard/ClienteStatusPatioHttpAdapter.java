...
@Component
@ConditionalOnProperty(name = "cloudport.modulo.yard.integracao", havingValue = "http", matchIfMissing = true)
public class ClienteStatusPatioHttpAdapter implements ClienteStatusPatio {

    private static final Logger LOGGER = LoggerFactory.getLogger(ClienteStatusPatioHttpAdapter.class);
    ...
    public ClienteStatusPatioHttpAdapter(
            RestTemplate restTemplate,
            @Value("${cloudport.integracoes.yard.base-url:http://localhost:8081}") String baseUrl,
    ...
    public Optional<StatusPatioResposta> consultarStatus(String authorizationHeader) {
        ...
        try {
            ...
            URI uri = UriComponentsBuilder.fromHttpUrl(baseUrl).path(statusPath).build(true).toUri();
            ResponseEntity<StatusPatioResposta> resposta =
                    restTemplate.exchange(uri, HttpMethod.GET, entity, StatusPatioResposta.class);
    ...
    private Optional<StatusPatioResposta> fallbackConsultarStatus(
