...
function WorkQueue({ queue, expanded, edit, priorities, busyKey, onToggle, onEdit, onAction, onInstruction, onDetails }) {
  ...
  return <article className={`queue ${derived ? 'queue-derived' : ''}`}><button className="queue-head" onClick={onToggle} aria-expanded={expanded}>
    ...
    {!derived && <><div className="queue-editor"><label>POW<input value={edit.pow} onChange={(event) => onEdit({ ...edit, pow: event.target.value })} /></label><label>Pool<input value={edit.poolOperacional} onChange={(event) => onEdit({ ...edit, poolOperacional: event.target.value })} /></label><button className="secondary" onClick={() => onAction('pow')}>Salvar POW/pool</button><label>Equipamento<input value={edit.equipamento} onChange={(event) => onEdit({ ...edit, equipamento: event.target.value })} /></label><button className="secondary" onClick={() => onAction('equipment')}>Salvar equipamento</button><label>Limite<input type="number" min="1" value={edit.limite ?? ''} onChange={(event) => onEdit({ ...edit, limite: event.target.value ? Number(event.target.value) : null })} /></label></div><div className="actions"><button className={queue.status === 'ATIVA' ? 'warning' : ''} onClick={() => onAction(queue.status === 'ATIVA' ? 'deactivate' : 'activate')}>{queue.status === 'ATIVA' ? 'Desativar' : 'Ativar'}</button><button disabled={busyKey === `dispatch-${queue.id}`} onClick={() => onAction('dispatch')}>Despachar</button></div></>}
...
function EquipmentPanel({ queues, onDetails }) {
  ...
  return <section className="panel"><div className="section-head"><div><span className="eyebrow">CHE</span><h2>Equipamentos e job lists</h2></div><span>{groups.length}</span></div><div className="equipment-grid">{groups.map((group) => <article key={group.id} className={`equipment-card ${group.id === 'SEM_EQUIPAMENTO' ? 'equipment-missing' : ''}`}><div className="equipment-head"><div><strong>{group.id}</strong><small>{group.queues.map((queue) => queue.pow || queue.identificador).join(' · ')}</small></div>{group.id === 'SEM_EQUIPAMENTO' && <ExceptionBadge type="SEM_EQUIPAMENTO" />}</div><div className="equipment-metrics"><span><b>{group.jobs.length}</b> jobs</span><span><b>{group.jobs.filter((job) => job.statusOrdem === 'EM_EXECUCAO').length}</b> executando</span><span><b>{group.queues.length}</b> filas</span></div><div className="equipment-jobs">{group.jobs.slice(0, 6).map((job) => <button className="job-chip" key={job.id} onClick={() => onDetails(job)}>#{job.sequenciaNavio ?? '—'} {job.codigoLote}<small>{job.statusOrdem}</small></button>)}{!group.jobs.length && <p className="empty">Sem jobs.</p>}</div></article>)}</div></section>;
}
...
function InstructionDrawer({ context, onClose }) {
  ...
  const { order, item, reservation, queue, relatedAlerts, relatedEvents, divergences } = context;
  ...
  </div><section className="drawer-section"><h3>Item de navio</h3>{item ? <pre>{JSON.stringify(item, null, 2)}</pre> : <p className="empty">Item não localizado.</p>}</section><section className="drawer-section"><h3>Divergências</h3><div className="list">{divergences.map((entry, index) => <article key={`${entry.tipo}-${index}`}><div><strong>{entry.tipo}</strong><span className={statusClass(entry.severidade || 'ALTA')}>{entry.severidade || 'ALTA'}</span></div><p>{entry.mensagem}</p></article>)}{!divergences.length && <p className="empty">Nenhuma divergência.</p>}</div></section><section className="drawer-section"><h3>Auditoria e eventos</h3><div className="timeline">{relatedEvents.map((event) => <article key={event.id}><time>{dateTime(event.criadoEm)}</time><div><strong>{event.tipoEvento}</strong><p>{event.descricao}</p><small>{event.usuario}</small></div></article>)}{!relatedEvents.length && <p className="empty">Nenhum evento relacionado.</p>}</div></section><section className="drawer-section"><h3>Alertas relacionados</h3><div className="list">{relatedAlerts.map((alert, index) => <article key={`${alert.tipo}-${index}`}><div><strong>{alert.tipo}</strong><span className={statusClass(alert.severidade)}>{alert.severidade}</span></div><p>{alert.mensagem}</p></article>)}{!relatedAlerts.length && <p className="empty">Nenhum alerta relacionado.</p>}</div></section></aside></div>;
}
...
function ControlRoom({ session, onLogout }) {
  ...
  const loadSnapshot = useCallback(async (id, silent = false) => {
    ...
    const promise = Promise.all([
      ...
      setItems(a); setSummary(b); setEvents(c); setIntegration(d); setReservations(e); setOrders(f); setQueues(g); setWorkQueues(h); setUncovered(i); setAlerts(j); setPlan(k); setLastUpdate(new Date());
  ...
  const context = useMemo(() => {
    ...
    if (!queue) divergences.push({ tipo: 'SEM_FILA', severidade: 'ALTA', mensagem: 'Work instruction sem fila persistente.' }); else { if (!queue.pow) divergences.push({ tipo: 'SEM_POW', severidade: 'ALTA', mensagem: 'Fila sem POW.' }); if (!queue.equipamento) divergences.push({ tipo: 'SEM_EQUIPAMENTO', severidade: 'ALTA', mensagem: 'Fila sem CHE.' }); }
    return { order, item, reservation, queue, relatedAlerts, relatedEvents, divergences };
