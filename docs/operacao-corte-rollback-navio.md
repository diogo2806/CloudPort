...
8. Carga Geral, Gate, Rail, Visibilidade, Yard, Navio e Navio Siderúrgico devem usar o mesmo emissor e decoder JWT no runtime consolidado.
...
Também devem ser construídas as duas formas suportadas da imagem backend e a imagem frontend:
...
O backend deve preparar o diretório persistente de documentos. O frontend deve receber a URL da API pelo mecanismo de configuração adotado no ambiente e manter fallback de SPA para `index.html`.
...
6. Carga Geral: conhecimento, item, lote, referência e movimentação controlada;
