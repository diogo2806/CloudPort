...
@ConditionalOnProperty(
        ...
        havingValue = "http",
        matchIfMissing = true)
public class AutenticacaoHttpAdapter implements AutenticacaoClient {

    private static final Logger LOGGER = LoggerFactory.getLogger(AutenticacaoHttpAdapter.class);
    ...
    public AutenticacaoHttpAdapter(
            RestTemplate restTemplate,
            @Value("${cloudport.security.autenticacao.base-url:http://localhost:8080}") String autenticacaoBaseUrl,
    ...
    public Optional<UserInfoResponse> buscarUsuario(String login, String authorizationHeader) {
        ...
        try {
            ...
            ResponseEntity<UserInfoResponse> response =
                    restTemplate.exchange(uri, HttpMethod.GET, entity, UserInfoResponse.class);
            return Optional.ofNullable(response.getBody());
        } catch (RestClientException ex) {
            LOGGER.debug("Falha ao buscar usuário {} no módulo de autenticação", login, ex);
            return Optional.empty();
        }
    }

    @SuppressWarnings("unused")
    private Optional<UserInfoResponse> fallbackBuscarUsuario(
            String login,
