...
function GateVehicleCard({ vehicle, selected, onSelect }) {
  ...
  return <button
    type="button"
    className={`gate-vehicle-card ${selected ? 'selected' : ''} ${problem ? 'problem' : ''}`}
    onClick={() => onSelect(vehicle)}
  >
    <span className="gate-vehicle-top">
      <strong>{vehicleTitle(vehicle)}</strong>
      <span className={`gate-sla gate-sla-${sla.level}`}>{sla.label}</span>
    </span>
...
function GateLane({ lane, selectedKey, onSelect }) {
  ...
  return <article className={`gate-lane gate-lane-${lane.flow.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`}>
    <header className="gate-lane-header">
      <div><span>{lane.stage}</span><h3>{lane.name}</h3></div>
      <strong>{total}</strong>
    </header>
    <div className="gate-lane-summary">
      <span>{lane.averageMinutes ? `Média ${lane.averageMinutes} min` : 'Fluxo em tempo real'}</span>
      <span>{lane.flow}</span>
    </div>
    <div className="gate-vehicle-list">
      {lane.vehicles.length
        ? lane.vehicles.map((vehicle) => <GateVehicleCard
          key={gateVehicleKey(vehicle) ?? `${lane.id}-${vehicleTitle(vehicle)}`}
          vehicle={vehicle}
          selected={selectedKey === gateVehicleKey(vehicle)}
          onSelect={onSelect}
        />)
        : <div className="gate-lane-empty">Pista livre</div>}
    </div>
  </article>;
}

function Journey({ vehicle }) {
  const journey = getGateJourney(vehicle);
  return <div className="gate-journey">
    {journey.map((stage, index) => <div className={`gate-stage gate-stage-${stage.state}`} key={stage.key}>
      <span className="gate-stage-marker">{stage.state === 'complete' ? '✓' : index + 1}</span>
...
function DocumentList({ documents }) {
  ...
  return <div className="gate-document-list">
    {documents.map((document, index) => <article key={document?.id ?? index}>
      <div><strong>{document?.tipoDocumento || document?.nomeArquivo || `Documento ${index + 1}`}</strong><small>{document?.numero || document?.nomeArquivo || 'Sem identificação'}</small></div>
      <StatusBadge value={document?.statusValidacao ?? document?.status ?? 'PENDENTE'} />
    </article>)}
  </div>;
}
...
function VehicleInspector({ vehicle, onPrint, printing }) {
  ...
  return <div className="gate-inspector">
    <header>
      <div><span className="eyebrow">Atendimento selecionado</span><h3>{vehicleTitle(vehicle)}</h3><p>{vehicle?.motorista || appointment?.motoristaNome || 'Motorista não informado'} · {vehicle?.transportadora || appointment?.transportadoraNome || 'Transportadora não informada'}</p></div>
      <div className="actions"><button type="button" onClick={() => onPrint(vehicle)} disabled={printing || !vehicle?.podeImprimirComprovante}>{printing ? 'Gerando...' : 'Imprimir / reimprimir EIR'}</button></div>
    </header>

    <div className="gate-inspector-facts">
      <span><small>Agendamento</small><strong>{appointment?.codigo ?? '—'}</strong></span>
      <span><small>Operação</small><strong>{appointment?.tipoOperacaoDescricao ?? appointment?.tipoOperacao ?? '—'}</strong></span>
      <span><small>Chegada prevista</small><strong>{formatDateTime(appointment?.horarioPrevistoChegada)}</strong></span>
      <span><small>Chegada real</small><strong>{formatDateTime(appointment?.horarioRealChegada)}</strong></span>
    </div>
...
export function GateVisualPage() {
  ...
  useEffect(() => {
    if (!selectedKey && (problems[0] || vehicles[0])) setSelectedKey(gateVehicleKey(problems[0] ?? vehicles[0]));
  }, [problems, vehicles, selectedKey]);

  async function printEir(vehicle) {
    const popup = window.open('', '_blank', 'width=850,height=950');
    if (!popup) {
      setError('O navegador bloqueou a janela de impressão do EIR. Libere pop-ups para o CloudPort.');
      return;
    }
  ...
  return <>
    <PageHeader
      eyebrow="Gate visual"
      title="Controle operacional das pistas"
      description="Filas, etapas, capacidade de janelas, SLA, documentos, avarias e transações problemáticas em uma única visão operacional."
      actions={<div className="gate-visual-toolbar"><label className="compact-field"><input type="checkbox" checked={autoRefresh} onChange={(event) => setAutoRefresh(event.target.checked)} />Atualização automática</label><span className="gate-live-clock">{now.toLocaleTimeString('pt-BR')}</span><button className="secondary" type="button" onClick={load} disabled={loading}>Atualizar agora</button></div>}
    />
    <Message type="error">{error}</Message>

    {loading && !panel ? <Loading label="Carregando pistas e atendimentos..." /> : <>
      <div className="metrics-grid">
        <MetricCard label="Veículos no fluxo" value={vehicles.length} detail={`${lanes.length} pista(s) ativa(s)`} />
        <MetricCard label="Em atendimento" value={panel?.veiculosAtendimento?.length ?? 0} detail="OCR, balança ou inspeção" />
        <MetricCard label="Janelas ocupadas" value={calendar.reduce((total, day) => total + day.windows.filter((item) => item.occupied > 0).length, 0)} detail={`${windows.length} janela(s) configurada(s)`} />
        <MetricCard label="Problemas / SLA" value={problems.length} detail="Exceções, documentos ou espera crítica" />
      </div>

      <Section title="Quadro visual das pistas" description="Cada veículo permanece selecionável durante toda a jornada, da fila de entrada à saída.">
        <div className="gate-lane-board">
          {lanes.length ? lanes.map((lane) => <GateLane key={lane.id} lane={lane} selectedKey={gateVehicleKey(selectedVehicle)} onSelect={(vehicle) => setSelectedKey(gateVehicleKey(vehicle))} />) : <EmptyState title="Nenhum veículo no fluxo atual" />}
        </div>
      </Section>

      <Section title="Calendário e capacidade das janelas" description="Ocupação calculada pelos agendamentos associados a cada janela.">
        {calendar.length ? <div className="gate-calendar">{calendar.slice(0, 7).map((day) => <article className="gate-day" key={day.date}><header><strong>{formatDay(day.date)}</strong><span>{day.windows.reduce((total, item) => total + item.occupied, 0)} agendamento(s)</span></header><div className="gate-window-grid">{day.windows.map((windowItem) => <div className={`gate-window ${windowItem.percentage >= 100 ? 'full' : ''}`} key={windowItem.id ?? `${day.date}-${windowItem.horaInicio}`}><div><strong>{formatHour(windowItem.horaInicio)}–{formatHour(windowItem.horaFim)}</strong><small>{windowItem.canalEntradaDescricao ?? windowItem.canalEntrada ?? 'Canal geral'}</small></div><span>{windowItem.occupied}/{windowItem.capacity || '—'}</span><div className="gate-capacity"><i style={{ width: `${windowItem.percentage}%` }} /></div></div>)}</div></article>)}</div> : <EmptyState title="Nenhuma janela configurada" />}
      </Section>

      <div className="gate-detail-grid">
        <Section title="Jornada gráfica e atendimento"><VehicleInspector vehicle={selectedVehicle} onPrint={printEir} printing={printing} /></Section>
        <Section title="Transações com problema" description="Priorização automática por SLA, exceções e falhas documentais.">
          {problems.length ? <div className="gate-problem-list">{problems.map((vehicle) => <button type="button" key={gateVehicleKey(vehicle)} onClick={() => setSelectedKey(gateVehicleKey(vehicle))}><div><strong>{vehicleTitle(vehicle)}</strong><small>{vehicle.filaOperacional} · {vehicleStatus(vehicle)}</small></div><span className={`gate-sla gate-sla-${classifyGateSla(vehicle.tempoFilaMinutos).level}`}>{classifyGateSla(vehicle.tempoFilaMinutos).label}</span></button>)}</div> : <EmptyState title="Nenhuma transação problemática" description="Todos os atendimentos estão dentro do SLA e sem exceções registradas." />}
        </Section>
      </div>
    </>}
  </>;
