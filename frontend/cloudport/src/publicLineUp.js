...
export function normalizeLineUp(payload) {
  ...
  return payload
    ...
    .sort((left, right) => {
      const leftTime = Date.parse(left.chegadaPrevista ?? '') || Number.MAX_SAFE_INTEGER;
      const rightTime = Date.parse(right.chegadaPrevista ?? '') || Number.MAX_SAFE_INTEGER;
