  ...
  useEffect(() => {
    ...
    const receiveSession = (event) => {
      ...
      try {
        ...
        if (!hasAnyRole(session, ...ALLOWED_ROLES)) throw new Error('Conta sem permissão operacional.');
...
function QuayMonitor({ data }) {
  ...
  return <div className="monitor-grid">
    ...
    <Metric label="Produtividade" value={`${number(data.movimentosPorHora, 2)} mov/h`} detail={`Conclusão ${dateTime(data.previsaoConclusao)}`} />
...
function VesselView({ rows = [] }) {
  ...
  return <div className="visual-grid">{rows.map((row) => <article className="visual-card" key={row.porao}>
    ...
    <small>{number(row.pesoOperadoToneladas, 1)} t de {number(row.pesoPlanejadoToneladas, 1)} t</small>
...
function IssuePanel({ title, rows = [], bottleneck = false }) {
  return <section className="panel">
    <div className="section-head"><h2>{title}</h2><span>{rows.length}</span></div>
    {!rows.length ? <p className="empty">Nenhum registro.</p> : <div className="list">{rows.map((row, index) => <article key={`${row.codigo || row.categoria}-${index}`}>
      <div><strong>{bottleneck ? row.codigo : `${row.codigoLote} · ${row.categoria}`}</strong><span className={statusClass(row.severidade)}>{row.severidade}</span></div>
      <p>{bottleneck ? row.causa : row.mensagem}</p>
      <small>{bottleneck ? `${row.recurso} · ${row.quantidadeAfetada} afetados · ${row.acaoRecomendada}` : `${row.planejado || '—'} → ${row.executado || '—'}`}</small>
    </article>)}</div>}
  </section>;
}
...
function OrdersTable({ orders, priorities, onPriority, onSuspend, onResume, busyKey, readOnly = false }) {
  ...
  return <div className="table-wrap"><table><thead><tr>
    ...
    <td>{readOnly ? priorities[order.id] ?? order.prioridadeOperacional ?? 0 : <div className="inline">
      <input type="number" min="0" value={priorities[order.id] ?? order.prioridadeOperacional ?? 0} onChange={(event) => onPriority(order, Number(event.target.value), false)} />
...
function WorkQueue({ queue, edit, expanded, busyKey, onToggle, onEdit, onAction, onInstruction }) {
  ...
  return <article className="queue">
    ...
    {expanded && <div className="queue-body">
      ...
      {!jobs.length ? <p className="empty">A fila não possui jobs.</p> : <div className="table-wrap nested"><table><thead><tr>
        <th>Lote</th><th>Status</th><th>Destino</th><th>Ações</th>
        ...
        <td>{job.destino || job.posicaoPlanejada || '—'}</td>
...
function StructuralValidation({ value, onChange, onValidate, result, busy }) {
  ...
  return <section className="panel">
    ...
    <div className="structural-grid">
      ...
      <label>Altura máxima<input type="number" min="1" value={value.alturaMaximaCamadas} onChange={(event) => set('alturaMaximaCamadas', event.target.value)} /></label>
      <label>Lashing a partir da camada<input type="number" min="1" value={value.exigirLashingAPartirCamada} onChange={(event) => set('exigirLashingAPartirCamada', event.target.value)} /></label>
      ...
      <label>Porões interditados<input value={value.poroesInterditados} onChange={(event) => set(' poroesInterditados'.trim(), event.target.value)} placeholder="2; 4" /></label>
    </div>
...
function ControlRoom({ session, onLogout }) {
  ...
  const [telemetryState, setTelemetryState] = useState('DESCONECTADO');
  ...
  const loadSnapshot = useCallback(async (id, silent = false) => {
    ...
    try {
      const [newSummary, newEvents, newIntegration, newReservations, newOrders, newWorkQueues, newUncovered, newAlerts, newAdvanced] = await Promise.all([
        api.obterResumo(id),
      ...
      setPriorities((current) => newOrders.reduce((acc, order) => ({ ...acc, [order.id]: current[order.id] ?? order.prioridadeOperacional ?? 0 }), {}));
  ...
  const scheduleRefresh = useCallback((id) => {
    ...
    refreshTimer.current = setTimeout(() => loadSnapshot(id, true).catch((reason) => setError(formatError(reason))), 300);
  }, [loadSnapshot]);

  useEffect(() => {
    api.listarVisitas().then((data) => {
      setVisits(data);
      setVisitId(data[0]?.id ?? null);
  ...
  useEffect(() => api.assinarTelemetriaEquipamentos({
    onState: setTelemetryState,
    onEvent: () => scheduleRefresh(visitId),
    onError: () => setTelemetryState('RECONECTANDO')
  }), [visitId, scheduleRefresh]);
  ...
  function changePriority(order, value, persist) {
    ...
    if (persist) action(`priority-${order.id}`, () => api.atualizarPrioridadeOrdemPatio(visitId, order.id, value), 'Prioridade atualizada.').then(() => loadSnapshot(visitId, true));
  }

  async function validateStructural() {
    const response = await action('structural', () => api.validarRestricoesEstruturais(visitId, {
      limitePesoPorPoraoToneladas: nullableNumber(structuralConfig.limitePesoPorPoraoToneladas),
      limitePesoPorCamadaToneladas: nullableNumber(structuralConfig.limitePesoPorCamadaToneladas),
      desequilibrioBombordoBoresteMaximoPercentual: nullableNumber(structuralConfig.desequilibrioBombordoBoresteMaximoPercentual),
      alturaMaximaCamadas: nullableNumber(structuralConfig.alturaMaximaCamadas),
      exigirLashingAPartirCamada: nullableNumber(structuralConfig.exigirLashingAPartirCamada),
  ...
  const filteredQueues = workQueues.filter((queue) =>
    ...
    && (!zoneFilter || `${queue.identificador} ${queue.berco} ${queue.blocoZona} ${queue.pow}`.toUpperCase().includes(zoneFilter.toUpperCase()))
  );
  const filteredAlerts = alerts.filter((alert) => !severityFilter || alert.severidade === severityFilter);
  const nextPhase = NEXT_PHASE[selectedVisit?.fase];

  return <div className="app">
    <header className="topbar">
      <div><span className="eyebrow">CloudPort</span><h1>Control Room Navio + Pátio</h1></div>
      <div className="stream-health"><span className={statusClass(streamState)}>Operação {streamState}</span><span className={statusClass(telemetryState)}>VMT {telemetryState}</span></div>
      <div className="top-actions"><span>{session.nome}</span><button className="secondary" onClick={() => loadSnapshot(visitId)}>Atualizar</button><button className="danger" onClick={onLogout}>Sair</button></div>
    </header>
    <main className="content">
      {error && <div className="message error" role="alert">{error}</div>}
      {success && <div className="message success">{success}</div>}
      <section className="panel selector">
        <label>Visita<select value={visitId ?? ''} onChange={(event) => setVisitId(Number(event.target.value))}>{visits.map((visit) => <option key={visit.id} value={visit.id}>{visit.codigoVisita} · {visit.navioNome || 'navio'}</option>)}</select></label>
        <div><span className={statusClass(selectedVisit?.fase)}>{selectedVisit?.fase || 'SEM_VISITA'}</span><small>{selectedVisit?.bercoAtual || selectedVisit?.bercoPrevisto || 'sem berço'} · atualização {dateTime(lastUpdate)}</small></div>
      ...
      <section className="metrics">
        <Metric label="Progresso" value={`${number(summary.percentualProgresso, 1)}%`} detail={`${summary.totalItensOperados}/${summary.totalItensPlanejados} itens`} />
        <Metric label="Peso operado" value={`${number(summary.pesoOperado, 1)} t`} detail={`${number(summary.pesoPlanejado, 1)} t planejadas`} />
      ...
      <section className="panel">
        <div className="section-head"><div><span className="eyebrow">Operação</span><h2>Ações, relatórios e filtros</h2></div><small>Atualização por eventos versionados.</small></div>
        <div className="actions">
          <button onClick={() => action('reserve', async () => { await api.gerarReservasPatio(visitId); await loadSnapshot(visitId, true); }, 'Reservas geradas.')}>Gerar reservas</button>
          <button onClick={() => action('orders', async () => { await api.gerarOrdensPatio(visitId); await loadSnapshot(visitId, true); }, 'Ordens geradas.')}>Gerar ordens</button>
          ...
          <button className="secondary" onClick={() => action('csv', () => api.baixarRelatorioCsv(visitId), 'CSV gerado.')}>Baixar CSV</button>
          <button className="secondary" onClick={() => action('pdf', () => api.baixarRelatorioPdf(visitId), 'PDF gerado.')}>Baixar PDF</button>
        </div>
      ...
      <section className="panel"><div className="section-head"><h2>Quay Monitor</h2></div><QuayMonitor data={advanced?.quayMonitor} /></section>
      ...
      <section className="panel"><div className="section-head"><div><span className="eyebrow">Equipment Control</span><h2>Equipamentos e job lists</h2></div><span>{advanced?.cheDetail?.length ?? 0}</span></div><CheDetail rows={advanced?.cheDetail} /></section>
      <div className="columns"><IssuePanel title="Previsão de gargalos" rows={advanced?.gargalos} bottleneck /><IssuePanel title="Comparação estiva, pátio e execução" rows={advanced?.divergencias} /></div>
      <StructuralValidation value={structuralConfig} onChange={setStructuralConfig} onValidate={validateStructural} result={structuralResult ?? advanced?.validacaoEstrutural} busy={busyKey === 'structural'} />

      <section className="panel"><div className="section-head"><h2>Work queues e job lists</h2><span>{filteredQueues.length}</span></div>{filteredQueues.map((queue) => <WorkQueue key={queue.id ?? queue.identificador} queue={queue} expanded={!!expanded[queue.id]} edit={edits[queue.id] ?? { pow: '', poolOperacional: '', equipamento: '', limite: null }} busyKey={busyKey} onToggle={() => setExpanded((current) => ({ ...current, [queue.id]: !current[queue.id] }))} onEdit={(edit) => setEdits((current) => ({ ...current, [queue.id]: edit }))} onAction={(type) => queueAction(queue, type)} onInstruction={instructionAction} />)}{!filteredQueues.length && <p className="empty">Nenhuma work queue encontrada.</p>}</section>
      <section className="panel"><div className="section-head"><h2>Ordens de pátio</h2><span>{filteredOrders.length}/{orders.length}</span></div><OrdersTable orders={filteredOrders} priorities={priorities} onPriority={changePriority} onSuspend={(order) => action(`suspend-${order.id}`, async () => { await api.suspenderOrdemPatio(visitId, order.id); await loadSnapshot(visitId, true); }, 'Ordem suspensa.')} onResume={(order) => action(`resume-${order.id}`, async () => { await api.retomarOrdemPatio(visitId, order.id); await loadSnapshot(visitId, true); }, 'Ordem retomada.')} busyKey={busyKey} /></section>
