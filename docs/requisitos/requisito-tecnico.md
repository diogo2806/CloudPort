...
Status: atualizado em 2026-07-18 após conclusão dos BUS1040, BUS1090 e BUS1170 na branch `main`.
...
| BUS1000 | Implementar ordens de trabalho de carga com ciclo de vida, itens, recursos e eventos. | Cada serviço possui ordem persistida com prioridade, janela, local, lotes, recursos, estados operacionais, eventos e cancelamento motivado. | ⬜ Pendente |
...
| Caminho completo | Método/campo/contrato | Como está | O que fazer |
|---|---|---|---|
| `backend/servico-carga-geral/src/main/java/br/com/cloudport/servicocargageral/controlador/CargaGeralControlador.java` | rotas de `/api/carga-geral` | Não existe contrato de ordem de trabalho operacional. | Criar endpoints para criar, liberar, atribuir, iniciar, registrar evento, concluir e cancelar ordem. |
| `backend/servico-carga-geral/src/main/java/br/com/cloudport/servicocargageral/servico/CargaGeralServico.java` | serviços públicos | Não existe agregado com prioridade, janela, local, recursos, itens e histórico. | Criar novo agregado sugerido: `OrdemTrabalhoCarga`. |
