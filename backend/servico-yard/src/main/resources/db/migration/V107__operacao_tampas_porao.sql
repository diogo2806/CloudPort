-- V107: operação persistida de tampas de porão e bloqueio de movimentos
...
CREATE TABLE movimento_container_navio (
    id BIGSERIAL PRIMARY KEY,
    estivagem_plan_id BIGINT NOT NULL,
    slot_navio_id BIGINT NOT NULL,
    ...
    criado_em TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    atualizado_em TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_movimento_container_navio_plano
        FOREIGN KEY (estivagem_plan_id) REFERENCES estivagem_plan(id),
    CONSTRAINT fk_movimento_container_navio_slot
        FOREIGN KEY (slot_navio_id) REFERENCES slot_navio(id),
    CONSTRAINT ck_movimento_container_navio_ordem
        CHECK (ordem_sequencia > 0 AND guindaste_id > 0),
    CONSTRAINT ck_movimento_container_navio_status
...
CREATE INDEX idx_movimento_container_navio_plano
    ON movimento_container_navio (estivagem_plan_id, ordem_sequencia, status);

CREATE UNIQUE INDEX uk_movimento_container_navio_slot_ativo
    ON movimento_container_navio (estivagem_plan_id, slot_navio_id)
